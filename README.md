...
Chrome / Edge のWebページをPNG保存、またはクリップボードへ直接コピーする Manifest V3 拡張です。タブバー、アドレスバーなど、ブラウザの外枠は入りません。ビルドや npm install は不要です。

| 操作 | Windows / Linux | macOS |
| --- | --- | --- |
| 表示中の領域をPNG保存 | Ctrl + Shift + Y | Command + Shift + Y |
| ページ全体をPNG保存 | Ctrl + Shift + U | Command + Shift + U |
| 表示中の領域をコピー | Alt + Shift + Y | Option + Shift + Y |
| ページ全体をコピー | Alt + Shift + U | Option + Shift + U |

ブラウザにフォーカスがある状態で使います。コピー用ショートカットを押すと、ポップアップや保存ダイアログを開かず、撮影したPNGがそのままクリップボードに入ります。拡張アイコンのポップアップからも4操作を実行できます。
...
アイコンのバッジは `…` が処理中、`↓` がダウンロード開始、`✓` がクリップボードへのコピー完了、`!` が失敗です。結果の詳細はアイコンを押して確認できます。`↓` は保存完了を保証する表示ではありません。保存の完了や中断はブラウザのダウンロード一覧（Windows は Ctrl + J）で確認してください。
...
- **ページ全体**: 撮影するタブに `chrome.debugger` で一時接続し、`Page.getLayoutMetrics` の `cssContentSize` を `Page.captureScreenshot` の `clip` に指定します。横方向・縦方向の画面外も含みます。通常は一括撮影し、画像内で同じ表示領域の反復を検出した場合は、表示領域サイズに分割して再撮影・結合します。ページのスクロール位置やウィンドウサイズは変更しません。
...
- 分割撮影への切り替えは自動です。Chromiumの画面外描画で同じ領域が縦に繰り返されるページを対象にしており、通常のページでは従来の一括撮影を使用します。
...
| clipboardWrite | 撮影したPNGをクリップボードに書き込み |
| downloads | PNGをダウンロードフォルダへ保存 |
| debugger | ページ全体のキャプチャ |
| offscreen | 反復画像の検出と分割画像の結合 |
| scripting | ショートカット操作時のアクティブなページでクリップボードAPIを実行 |
| storage | 直近の実行結果をブラウザのセッション内に保持 |

`debugger` は強い権限で、インストール時にブラウザが警告を表示します。撮影操作時だけ使用します。外部サーバーへの通信、解析サービス、常駐コンテンツスクリプトはありません。画像はダウンロードAPIまたはOSのクリップボードに渡し、履歴ストレージには保存しません。
...
自動テストではAPIをモックし、撮影モードの分離、対象ウィンドウ、CSS寸法、反復検出、分割範囲と画像結合への切り替え、デバッグ接続の後始末、連打・並行実行、保存エラーを検証します。実ブラウザでの画像品質は以下で確認してください。
