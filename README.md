...
- **ページ全体**: 撮影するタブに `chrome.debugger` で一時接続し、`Page.getLayoutMetrics` の `cssContentSize` を `Page.captureScreenshot` の `clip` に指定します。横方向・縦方向の画面外も含みます。短いページでは、境界だけが欠ける描画不良を避けるため、一時的に表示領域をページ全体の高さへ広げた撮影も試します。長いページなどで画像内に同じ表示領域の反復が見つかった場合は、順にスクロールして表示領域を重ねて撮影・結合します。終了後は元の表示領域とスクロール位置に戻します。
...
- 再撮影への切り替えは自動です。分割撮影中はページのスクロール位置が一時的に変わり、固定表示の要素が複数箇所に写る場合があります。ページ自体を必要な位置までスクロールできない場合はエラーを表示します。
...
出力先は `dist/korochin-pageshot-v1.1.2.zip` です。ZIPにはバージョン付きのトップフォルダがあり、展開後にそのフォルダをChrome / Edgeの「パッケージ化されていない拡張機能を読み込む」で指定できます。`tests`、GitHub Actions、開発用ファイルは含みません。
