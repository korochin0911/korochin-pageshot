...
ブラウザにフォーカスがある状態で使います。画像コピー用ショートカットを押すと、ポップアップや保存ダイアログを開かず、撮影したPNGがそのままクリップボードに入ります。「ページタイトルをコピー」は表示中のページの `document.title` をテキストとしてコピーします。拡張アイコンのポップアップからも各操作を実行できます。
...
Chromiumでは拡張機能が初期値を指定できるショートカットが4個までのため、「ページタイトルをコピー」には初期キーを設定していません。上記の画面で任意のキーを割り当ててください。

## 撮影方式と制約

- **表示領域**: `chrome.tabs.captureVisibleTab()` を使い、現在のスクロール位置で見えているページ部分を保存します。
- **ページ全体**: 撮影するタブに `chrome.debugger` で一時接続し、`Page.getLayoutMetrics` の `cssContentSize` を `Page.captureScreenshot` の `clip` に指定します。横方向・縦方向の画面外も含みます。通常は一括撮影し、画像内で同じ表示領域の反復を検出した場合は、ページを順にスクロールして表示領域を重ねて撮影・結合します。描画待ちを入れ、つなぎ目の内容が画面端だけに現れないようにします。終了後は元のスクロール位置に戻します。ウィンドウサイズは変更しません。
...
- 分割撮影への切り替えは自動です。Chromiumの画面外描画で同じ領域が縦に繰り返されるページを対象にしており、通常のページでは従来の一括撮影を使用します。分割撮影中はページのスクロール位置が一時的に変わり、固定表示の要素が複数箇所に写る場合があります。ページ自体を必要な位置までスクロールできない場合はエラーを表示します。
...
| clipboardWrite | 撮影したPNGまたはページタイトルをクリップボードに書き込み |
| downloads | PNGをダウンロードフォルダへ保存 |
| debugger | ページ全体のキャプチャ |
| offscreen | 反復画像の検出と分割画像の結合 |
| scripting | ショートカット操作時のアクティブなページで画像・タイトルのクリップボードAPIを実行 |
...
6. 「ページタイトルをコピー」にショートカットを割り当て、実行後にテキストエディターへ貼り付けると `PageShot キャプチャ確認ページ` になることを確認します。
7. 全体撮影後にデバッグ通知が消えること、失敗時にポップアップに理由が出ることを確認します。
...
出力先は `dist/korochin-pageshot-v1.1.1.zip` です。ZIPにはバージョン付きのトップフォルダがあり、展開後にそのフォルダをChrome / Edgeの「パッケージ化されていない拡張機能を読み込む」で指定できます。`tests`、GitHub Actions、開発用ファイルは含みません。
