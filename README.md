...
- **ページ全体**: 撮影するタブに `chrome.debugger` で一時接続し、`Page.getLayoutMetrics` の `cssContentSize` を `Page.captureScreenshot` の `clip` に指定します。横方向・縦方向の画面外も含みます。短いページや表示の反復が検出されたページでは、サイズ制限内なら一時的に表示領域をページ全体の高さへ広げた撮影を試します。これが使えない場合は、順にスクロールして表示領域を重ねて撮影・結合します。終了後は元の表示領域とスクロール位置に戻します。
